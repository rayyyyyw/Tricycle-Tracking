...
export default function BookingChat({
        ...
        (async () => {
            try {
                const [tokenRes, messagesRes] = await Promise.all([
                    fetch(`/api/bookings/${bookingId}/chat-token`, {
                        credentials: 'include',
                    }),
                ...
                const [tokenJson, messagesJson] = await Promise.all([
                    tokenRes.json(),
                    messagesRes.json(),
                ]);
                if (cancelled) return;
    ...
    useEffect(() => {
            ...
            (async () => {
                ...
                try {
                    ...
                    await Promise.all([
                        ...
                        fetch(
                            `${base}/api/bookings/${bookingId}/messages/mark-read`,
                            opts,
                        ),
                    ]);
                    socket.emit(
        ...
        socket.on('message_delivered', (data: { message_ids: number[] }) => {
            const ids = new Set(
                (data?.message_ids || []).map(Number).filter(Boolean),
            );
        ...
        socket.on('message_read', (data: { message_ids: number[] }) => {
            ...
            setMessages((prev) =>
                prev.map((m) =>
                    m.sender_id === currentUserId && ids.has(m.id)
                        ? {
                              ...m,
                              read_at: ts,
                              delivered_at: m.delivered_at ?? m.created_at,
                          }
                        : m,
                ),
            );
    ...
    if (loading) {
        return embedded ? (
            ...
            <div className="flex items-center justify-center gap-2 rounded-lg border border-emerald-200 bg-white p-4 text-muted-foreground dark:border-emerald-800/50 dark:bg-gray-800">
                <Loader2 className="h-4 w-4 animate-spin" />
                <span className="text-sm">Loading chat…</span>
            </div>
        );
    ...
    if (error && !token) {
        return embedded ? (
            <div className="py-6 text-center text-sm text-amber-600 dark:text-amber-400">
                {error}
            </div>
        ) : (
            <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-4 text-center text-sm text-amber-800 dark:border-amber-800/50 dark:bg-amber-950/20 dark:text-amber-200">
                {error}
            </div>
        );
    }
    ...
    const messagesSection = (
        <div
            ref={listRef}
            className={`flex-1 space-y-2 overflow-y-auto p-3 ${embedded ? 'max-h-[280px] min-h-[180px]' : 'max-h-[240px] min-h-[160px]'}`}
        >
            {messages.length === 0 ? (
    ...
    const typingUserName =
        typingUserId != null ? (otherMessage?.sender_name ?? null) : null;
        ...
        >
            {typingUserId != null && (
                <div className="flex animate-in items-center gap-1.5 text-xs text-muted-foreground duration-200 fade-in">
                    ...
                    <span className="inline-flex gap-0.5" aria-hidden="true">
                        <span className="animate-typing-dot h-1 w-1 rounded-full bg-current opacity-60" />
                        <span className="animate-typing-dot-2 h-1 w-1 rounded-full bg-current opacity-60" />
                        <span className="animate-typing-dot-3 h-1 w-1 rounded-full bg-current opacity-60" />
                    </span>
                </div>
            )}
            <div className="flex gap-2">
                ...
                <Button
                    ...
                    onClick={handleSend}
                    disabled={
                        !input.trim() || sending || !connected || connectError
                    }
            ...
            {!embedded && connectingSlow && !connectError && (
                <p className="text-[11px] text-emerald-600 dark:text-emerald-400">
                    Server may be waking up – this can take 1–2 minutes. Please
                    wait, we&apos;ll keep retrying.
                </p>
            )}
    ...
    return (
        <div className="flex flex-col overflow-hidden rounded-lg border border-emerald-200 bg-white dark:border-emerald-800/50 dark:bg-gray-800">
            <div className="flex items-center gap-2 border-b border-emerald-200/50 bg-emerald-50/30 px-4 py-2 dark:border-emerald-800/30 dark:bg-emerald-950/20">
                ...
                {connected ? (
                    ...
                    <span
                        className="ml-auto text-xs text-amber-600 dark:text-amber-400"
                        title={
                            typeof window !== 'undefined' &&
                            /^localhost$|^127\.0\.0\.1$/i.test(
                                window.location.hostname,
                            )
                                ? 'Start the chat server: npm run socket'
