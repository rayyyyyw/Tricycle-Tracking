...
const REALTIME_PING_INTERVAL_MS = 10_000; // 10 seconds when on active ride (for real-time map)
...
/**
 * Pings location more frequently (every 10s) when user is on an active ride so passenger/driver maps update in real time.
 * Call with enabled: true when viewing an accepted or in_progress booking.
 */
export function useRealtimeLocationPing(enabled: boolean) {
    useEffect(() => {
        ...
        tick();
        const interval = setInterval(tick, REALTIME_PING_INTERVAL_MS);
        return () => clearInterval(interval);
    }, [enabled]);
}
